...
function parseFingering(fingering, firstFret, rootNoteStr, fretRange) {
    const marks = [];
    const notes = [];
    const intervals = [];
...
// --- New functions for index14.html ---

function addNote(note) {
    const chordInput = document.getElementById('chordInput');
    const currentNotes = chordInput.value.trim();
    if (currentNotes) {
        chordInput.value += ' ' + note;
    } else {
        chordInput.value = note;
    }
}
...
function generateChord() {
    console.log('generateChord called');
    let notesInput = document.getElementById('chordInput').value;
    const rootNote = document.getElementById('rootNote').value;
    const chordType = document.getElementById('chord-type').value;
    const startFret = parseInt(document.getElementById('firstFret').value);
    const fretRange = parseInt(document.getElementById('fretRange').value);

    const outputContainer = document.getElementById('output');
    outputContainer.innerHTML = '';

    let currentNotes = document.getElementById('chordInput').value.trim();

    // Always generate notes from dropdowns if rootNote and chordType are selected
    if (rootNote && chordType) {
        const chordTones = {
            'major': [0, 4, 7], 'minor': [0, 3, 7], 'diminished': [0, 3, 6],
            'augmented': [0, 4, 8], 'dom7': [0, 4, 7, 10], 'maj7': [0, 4, 7, 11],
            'min7': [0, 3, 7, 10]
        };
        const rootNum = NOTE_MAP[rootNote];
        const intervals = chordTones[chordType] || [0, 4, 7];
        currentNotes = intervals.map(interval => NOTE_NAMES[(rootNum + interval) % 12]).join(' ');
        document.getElementById('chordInput').value = currentNotes;
        console.log('Generated notes from dropdowns:', currentNotes);
    } else if (!currentNotes) { // If dropdowns not selected AND input is empty, show error
        outputContainer.innerHTML = '<p class="error-message">Please select a root note and chord type, or enter notes manually.</p>';
        console.log('Error: Root or Chord Type missing for auto-generation, and notes input is empty.');
        return;
    }

    // Now use currentNotes (either from dropdowns or manual input) for diagram generation
    const chordNotes = parseChordInput(currentNotes)[0];
    console.log('Parsed Chord Notes (numeric):', chordNotes);
    const mockFingering = generateFingeringPattern(chordNotes, startFret, rootNote);
    console.log('Generated Fingering Pattern:', mockFingering);

    const chordData = parseFingering(mockFingering, startFret, rootNote, fretRange);
    console.log('Chord Data for SVG:', chordData);
    const labelingInfo = fretLabeler.labelChordFrets(chordData);
    console.log('Labeling Info:', labelingInfo);

    const diagramsContainer = document.createElement('div');
    diagramsContainer.className = 'chord-diagrams';
    console.log('Diagrams container created.');
    ...
    types.forEach((diagram, index) => {
        console.log(`Generating ${diagram.type} diagram.`);
        ...
        console.log(`${diagram.type} diagram appended to container.`);
    });

    outputContainer.appendChild(diagramsContainer);
    console.log('All diagrams appended to output.');
}
